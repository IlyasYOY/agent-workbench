...
Before editing a personal repo, inspect that repo's `AGENTS.md` and the relevant
files. Preserve dirty user changes. Do not modify the current project as a
fallback when the request clearly targets another personal repo.

Canonical personal repositories:

- Dotfiles: `~/Projects/IlyasYOY/dotfiles`

When the user asks from any project to work on shell config, workstation
bootstrap, Homebrew manifests, terminal or desktop config, treat the target as
the dotfiles repo unless the prompt names another path.

- Neovim workbench: `~/Projects/IlyasYOY/nvim-workbench`

When the user asks to work on Neovim configuration, snippets, personal plugin
registration, or Neovim runtime checks, treat the target as the nvim-workbench
