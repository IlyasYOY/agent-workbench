...
name: setup-opencode
description: >-
  Configure ~/.config/opencode/opencode.json from this agent-workbench repository.
...
1. Confirm the current Git root contains this skill at
   ...
   is intentionally limited to the agent-workbench repository.
...
5. Handle symlinks before comparing values:
   - If the target links to the legacy dotfiles
     `config/opencode/opencode.json`, propose replacing it with a regular
     local file. Preserve a readable resolved file as the backup; for a
     dangling link, preserve the link itself and record its target.
